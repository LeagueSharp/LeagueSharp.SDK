namespace LeagueSharp.SDK.Core.UI.Skins.Default
{
    using LeagueSharp.SDK.Core.Enumerations;
    using LeagueSharp.SDK.Core.Math;
    using LeagueSharp.SDK.Core.UI.Values;

    using SharpDX;
    using SharpDX.Direct3D9;

    /// <summary>
    /// A default implementation of IDrawableKeyBind
    /// </summary>
    public class DefaultKeyBind : DefaultComponent, IDrawableKeyBind
    {
        /// <summary>
        /// Draws a MenuKeyBind
        /// </summary>
        /// <param name="component">MenuKeyBind</param>
        public void Draw(MenuKeyBind component)
        {
            var centerY =
                (int)
                GetContainerRectangle(component.Container)
                    .GetCenteredText(null, component.Container.DisplayName, CenteredFlags.VerticalCenter)
                    .Y;
            DefaultSettings.Font.DrawText(
                MenuManager.Instance.Sprite,
                component.Interacting ? "Press a key" : component.Container.DisplayName,
                (int)(component.Container.Position.X + DefaultSettings.ContainerTextOffset),
                centerY,
                DefaultSettings.TextColor);

            if (!component.Interacting)
            {
                string keyString = "[" + component.Key + "]";
                DefaultSettings.Font.DrawText(
                    MenuManager.Instance.Sprite,
                    keyString,
                    (int)
                    (component.Container.Position.X + component.Container.MenuWidth - DefaultSettings.ContainerHeight
                     - CalcWidthText(keyString) - DefaultSettings.ContainerTextOffset),
                    centerY,
                    DefaultSettings.TextColor);
            }

            var line = new Line(Drawing.Direct3DDevice)
                           { Antialias = false, GLLines = true, Width = DefaultSettings.ContainerHeight };
            line.Begin();
            line.Draw(
                new[]
                    {
                        new Vector2(
                            (component.Container.Position.X + component.Container.MenuWidth
                             - DefaultSettings.ContainerHeight) + DefaultSettings.ContainerHeight / 2f,
                            component.Container.Position.Y + 1),
                        new Vector2(
                            (component.Container.Position.X + component.Container.MenuWidth
                             - DefaultSettings.ContainerHeight) + DefaultSettings.ContainerHeight / 2f,
                            component.Container.Position.Y + DefaultSettings.ContainerHeight)
                    },
                component.Active ? new ColorBGRA(0, 100, 0, 255) : new ColorBGRA(255, 0, 0, 255));
            line.End();
            line.Dispose();

            var centerX =
                (int)
                new Rectangle(
                    (int)
                    (component.Container.Position.X + component.Container.MenuWidth - DefaultSettings.ContainerHeight),
                    (int)component.Container.Position.Y,
                    DefaultSettings.ContainerHeight,
                    DefaultSettings.ContainerHeight).GetCenteredText(
                        null,
                        component.Active ? "ON" : "OFF",
                        CenteredFlags.HorizontalCenter).X;
            DefaultSettings.Font.DrawText(
                MenuManager.Instance.Sprite,
                component.Active ? "ON" : "OFF",
                centerX,
                centerY,
                DefaultSettings.TextColor);
        }

        /// <summary>
        /// Gets the On/Off boundaries
        /// </summary>
        /// <param name="component">MenuKeyBind</param>
        /// <returns>Rectangle</returns>
        public Rectangle ButtonBoundaries(MenuKeyBind component)
        {
            return
                new Rectangle(
                    (int)
                    (component.Container.Position.X + component.Container.MenuWidth - DefaultSettings.ContainerHeight),
                    (int)component.Container.Position.Y,
                    DefaultSettings.ContainerHeight,
                    DefaultSettings.ContainerHeight);
        }

        /// <summary>
        /// Gets the keybind boundaries
        /// </summary>
        /// <param name="component">MenuKeyBind</param>
        /// <returns>Rectangle</returns>
        public Rectangle KeyBindBoundaries(MenuKeyBind component)
        {
            return GetContainerRectangle(component.Container);
        }

        /// <summary>
        /// Gets the width of the MenuKeyBind
        /// </summary>
        /// <param name="keyBind">MenuKeyBind</param>
        /// <returns>int</returns>
        public int Width(MenuKeyBind keyBind)
        {
            return
                (int)
                (DefaultSettings.ContainerHeight + CalcWidthText("[" + keyBind.Key + "]")
                 + DefaultSettings.ContainerTextOffset);
        }
    }
}